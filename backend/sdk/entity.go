...
// 初始化 gRPC 客户端单例
func getClient() (sink2.DataHubClient, error) {
	var stub sink2.DataHubClient
	once.Do(func() {
		...
		stub = sink2.NewDataHubClient(conn)
	})
	return stub, initErr
}
...
func (r *ResultService) send(records []Record) error {
	...
	if err := stream.Send(&sink2.DoSinkRequest{
		SinkId: r.sinkID,
		Data:   data,
	}); err != nil {
		return err
